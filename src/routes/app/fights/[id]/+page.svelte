<script lang="ts">
	...
	$: shareableLink = `${$page.url.origin}/app/fights/${$page.params.id}/respond`;
	$: daysAgoIntl = () => {
	...
	const deleteFight = async() => {
		try{
			await pb.collection('fights').delete($page.params.id);
	...
	<div class="flex h-full flex-grow flex-col justify-between bg-background">
		<div class="flex-grow">
			...
			<div class="max-container">
				...
				>
					<h1 class="flex-grow font-heading text-lg font-semibold">
