	...
	<div class="flex h-full w-full flex-col overflow-hidden">
		...
		{:else}
			<div class="max-container pb-16">
				...
				<div class="rounded-b-xl px-2 pb-3 pt-2 mt-6">
					<h1 class="font-bold text-lg leading-tight mb-1">
						{data.analysis.title}
					</h1>
					...
					<div class="mb-6">
						<div class="flex flex-wrap -mx-2">
							{#each dials as dial}
							{#if dial.type === 'dial'}
							<Dial value={data.analysis[dial.slug]} min={dial.min} max={dial.max} label={dial.label} class="w-1/3 md:w-1/4" />
							{:else if dial.type === 'number'}
							<Counter value={data.analysis[dial.slug]} min={dial.min} max={dial.max} label={dial.label} class="w-1/3 md:w-1/4" />
							{/if}
						{/each}
					</div>
					</div>
					
					<div class="flex flex-col gap-4 bg-offwhite rounded-lg p-4">
						...
						<div class="flex flex-col gap-2">
							...
							<div class="text-sm text-gray-800 flex flex-wrap gap-y-1">
					...
					<div class="flex flex-col gap-4 bg-neutral-400 rounded-lg p-4 mt-6 {chatExpanded ? 'max-h-auto' : 'max-h-40'} overflow-y-hidden relative">
						<button onclick={() => chatExpanded = !chatExpanded} class="absolute bottom-2 left-1/2 transform -translate-x-1/2 text-red-600 z-10 {chatExpanded ? 'rotate-180' : ''}">
							<ChevronDown class="size-4 text-white" />
						</button>
						<div class="absolute inset-0 bg-gradient-to-b from-transparent to-neutral-400 {chatExpanded ? 'opacity-0' : 'opacity-100'}"></div>
						<div class="flex flex-col gap-2">
							<h2 class="font-bold">
								Chat
							</h2>
