<script lang="ts">
	...
	let { userSeeds, plantsAvailable, onPurchase }: Props = $props();
...
<!-- Seed count button -->
<button
	class="flex items-center gap-2 rounded-full border border-white bg-offwhite px-3 py-0.5"
	onclick={handleButtonClick}
>
	<span class="text-base">🌰</span>
	<span class="text-black font-bold">{getTotalSeeds()}</span>
</button>

<!-- Shop Drawer -->
<Drawer.Root open={showShop} onOpenChange={(open) => !open && closeShop()} snapPoints={['500px', 1]} bind:activeSnapPoint>
