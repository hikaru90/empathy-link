		...
		<div class="px-4 pb-3 pt-2">
			...
			{:else}
				...
				<div class="flex justify-center pt-4">
					{#if !showMore}
					<button on:click={() => (showMore = true)} class="text-xs rounded-full border border-needs-foreground/20 px-2 py-1">Mehr anzeigen</button>
					{:else}
					<button on:click={() => (showMore = false)} class="text-xs rounded-full border border-needs-foreground/20 px-2 py-1">Weniger anzeigen</button>
					{/if}
				</div>
			{/if}
		</div>
